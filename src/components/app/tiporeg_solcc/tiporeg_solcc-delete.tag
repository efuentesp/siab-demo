...
<tiporeg_solcc-delete>
	<page id="tiporeg_solcc-delete" title="Eliminar TipoReg_SolCCs">
		<formbox title="Datos del TipoReg_SolCC" icon="fa fa-check-circle-o" action="delete" return="tipoReg_SolCC-admin">
			<div class="row">
				<div class="col-md-4">
				<inputbox id="TRegSolCCNum" type="number" pattern="[0-9]" onkeypress="return event.charCode >= 48 && event.charCode <= 57" label="Número Tipo Registro" value="" placeholder="Número" required=true disabled=true minsize=3 maxsize=100 />
				</div>
			
			<p hidden>1</p>
				<div class="col-md-4">	
				
				<inputbox id="TRegSolCCDec" type="text" label="Descripción Tipo Registro" value="" placeholder="Descripción" required=true disabled=true />
				</div>
